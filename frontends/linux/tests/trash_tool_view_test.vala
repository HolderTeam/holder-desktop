...
private void test_refresh_without_project_shows_select_message() {
    ...
    assert(view.scope_text_for_tests() == "Projects / (none) / Trash");
    assert(view.empty_text_for_tests() == "Select a project to view trash.");
    assert(!view.empty_trash_sensitive_for_tests());
...
private void test_refresh_with_items_updates_scope_and_state() {
    ...
    assert(view.scope_text_for_tests() == "Projects / Project 1 / Trash");
...
private void test_filter_selection_updates_type_param() {
    ...
    view.set_filter_index_for_tests(1);
    assert(wait_for_condition(() => api.last_trash_type == "card"));

    view.set_filter_index_for_tests(2);
...
private void test_refresh_failure_emits_error_and_empty_state() {
    ...
    assert(view.empty_visible_for_tests());
    assert(view.empty_text_for_tests() == "Failed to load trash.");
    assert(!view.empty_trash_sensitive_for_tests());
}
...
private void test_restore_hard_delete_and_empty_actions() {
    ...
    view.restore_item_for_tests.begin(item, (obj, res) => {
        view.restore_item_for_tests.end(res);
        restore_done = true;
    });
    ...
    view.hard_delete_item_for_tests.begin(item, (obj, res) => {
        view.hard_delete_item_for_tests.end(res);
    ...
    view.empty_trash_for_tests.begin("p1", (obj, res) => {
        view.empty_trash_for_tests.end(res);
        empty_done = true;
    });
